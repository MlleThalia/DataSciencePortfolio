...
import scripts.cone as cone
import scripts.baselines as baselines
import scripts.parambath as parambath
import scripts.bisection as bisection

def experiment(argv):
    """ Hanlde exeperience corresponding to options """

    dataset = np.load(argv.dataset, allow_pickle=True)['dataset'].item()
    ...
    for fold_i in argv.fold_grid:
        ...
        for hparam in grid_hparam:
            ...
            if not argv.save_predictions and "predictions" in results[hparam]: #remove predictions to save space
