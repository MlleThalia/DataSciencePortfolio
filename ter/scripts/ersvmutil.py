...
# OR-Tools version of calc_nu_min
def calc_nu_min(xmat, y):
    m, n = xmat.shape
    ...
    # Variables
    z = [solver.NumVar(0, solver.infinity(), f'z_{i}') for i in range(m)]
    t = solver.NumVar(0, solver.infinity(), 't')

    # Constraint 1: sum(z_i) = 2
    solver.Add(sum(z) == 2)

    # Constraint 2: sum(y_i * z_i) = 0
    solver.Add(sum(y[i] * z[i] for i in range(m)) == 0)

    # Constraint 3: feature constraints
    constraint_mat = np.dot(np.diag(y), xmat).T
    for i in range(n):
        solver.Add(sum(constraint_mat[i][j] * z[j] for j in range(m)) == 0)
