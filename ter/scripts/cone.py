...
def run_algo(data, nb_class, hparam, argv):
    ...
    while max_fm >= best_fm and step < argv.max_step:
        ...
        outputs["t_values"][step] = next_t_val # t value for this step
        class_w = compute_weights(outputs["t_values"][step], nb_class, argv.beta) # compute class weights
...
def compute_weights(t_value, nb_class, beta=1.0): #calcule les poids définis par la fonction a(t)
